...
- **`run_security_playbook` and `scan_vulnerabilities` also target tables that do not exist** (`sn_si_playbook_execution`, `sn_vul_scan_request`) and cannot work. Unlike the fixes above these are not table renames: starting a PAD playbook is an API call (`sn_pd` process API), not a `sys_pd_context` insert, and an on-demand VR scan is a `sn_vul_scan` record plus `sn_vul_m2m_scan_configuration_item` links, not a single row. Both need a redesign against those models and live verification with a real playbook / real CIs, so they are left failing loudly rather than patched to look alive.
