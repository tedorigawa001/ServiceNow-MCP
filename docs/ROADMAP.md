...
## 11. USEM 修復ワークフロー補完 ✅ 完了(2026-07-12)
...
| 11-3 ✅ | `list_remediation_tasks` / `get_remediation_task` の是正 — 両テーブル横断に変更。list は 2 テーブルを並列照会して `source_table` 付きでマージ(`by_table` に内訳)、get は VUL 番号→`sn_vul_vulnerability` 直行、その他は task_number→number の順でフォールバック。**2026-07-12 実装・実機検証済み**(PDI 実測: legacy 132 件 vs エンジン側 3,065 件 — 従来の死角を解消) | 既存修正 | 中 |
| 11-4 ✅ | `get_finding_grouping_status` — VI の `is_in_group` / 所属 RT / アクティブルール / RT の `auto_vi_refresh` を 1 コールで返す診断ツール。診断順: vulnerability 空 → cmdb_ci 空 → アクティブルール無し → ルール不一致/未発火(auto_vi_refresh 案内含む)。**2026-07-12 実装・実機検証済み**(sn_sec_rem_task_rule の field_1 取得可、grouped/not-grouped 両ケースの入力を確認) | 新規ツール | 中 |
