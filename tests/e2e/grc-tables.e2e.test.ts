 ...
 *
 * GRC is an optional plugin. Every test gates on its table via
 * skipUnlessTables so a PDI without GRC reports "skipped", not "failed".
 */
...
e2eDescribe('E2E – GRC tables (read-only)', () => {
  ...
  describe('sn_audit_engagement', () => {
    it('lists audit engagements', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_audit_engagement');
      const result = await executeGrcAuditToolCall(client, 'list_audit_engagements', { limit: 5 });
      expect(result.count).toBeGreaterThanOrEqual(0);
      expect(Array.isArray(result.records)).toBe(true);
    });

    it('fetches a single engagement when one exists', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_audit_engagement');
  ...
  describe('sn_risk_risk', () => {
    ...
    it('fetches a single risk when one exists', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_risk_risk');
  ...
  describe('sn_grc_profile (entities)', () => {
    it('lists GRC entities', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_grc_profile');
      const result = await executeGrcComplianceToolCall(client, 'list_grc_entities', { limit: 5 });
      expect(result.count).toBeGreaterThanOrEqual(0);
      expect(Array.isArray(result.records)).toBe(true);
    });

    it('fetches a single entity when one exists', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_grc_profile');
  ...
  describe('sn_grc_indicator', () => {
    it('lists GRC indicators', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_grc_indicator');
      const result = await executeGrcIndicatorToolCall(client, 'list_grc_indicators', { limit: 5 });
      expect(result.count).toBeGreaterThanOrEqual(0);
      expect(Array.isArray(result.records)).toBe(true);
    });

    it('fetches a single indicator when one exists', async (ctx) => {
      await skipUnlessTables(ctx, client, 'sn_grc_indicator');
