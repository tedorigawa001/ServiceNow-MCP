 ...
 *   2. OAuth grant type (client_credentials / password)
 *   3. Credentials
 *   4. Connection test
...
export async function runSetup(options: { add?: boolean } = {}): Promise<void> {
  ...
  sectionLabel('OAuth 2.0 is the only supported authentication method');
  console.log('');
  ...
  console.log('');
  const clientId = await input({ message: brand('?') + ' Client ID' + brand(':') });
  const clientSecret = await password({ message: brand('?') + ' Client Secret' + brand(':'), mask: '•' });
  console.log('');

  const grantType = await select<'client_credentials' | 'password'>({
    message: brand('?') + ' OAuth grant type' + brand(':'),
    choices: [
      {
        name: `${accent('🔒')} Client Credentials ${dim('— client_id + secret only (recommended)')}`,
        value: 'client_credentials',
      },
      {
        name: `${brand('👤')} Password Grant ${dim('— also provide username + password')}`,
        value: 'password',
      },
    ],
  });
  ...
  let oauthPassword: string | undefined;
  if (grantType === 'password') {
    sectionLabel('Service account credentials for password grant');
    console.log('');
    oauthUsername = await input({ message: brand('?') + ' Username' + brand(':') });
    oauthPassword = await password({ message: brand('?') + ' Password' + brand(':'), mask: '•' });
  }
  ...
  let connected = false;
  while (!connected) {
    const { ok } = await testConnection(instanceUrl, {
      clientId,
      clientSecret,
      oauthUsername,
      oauthPassword,
    ...
    if (action === 'creds') {
      console.log('');
      // Re-prompt all OAuth credentials
      // (clientId/clientSecret are const so use separate re-prompt vars and break out of loop)
      break;
    }
  }
